...
int wmain(int argc, wchar_t** argv)
{
    if ((argc != 3 && argc != 4) || wcscmp(argv[1], L"--output") != 0 || !argv[2][0] ||
        (argc == 4 && wcscmp(argv[3], L"--inject-required-failure") != 0 && wcscmp(argv[3], L"--inject-font-failure") != 0 && wcscmp(argv[3], L"--inject-adapter-failure") != 0 && wcscmp(argv[3], L"--inject-fit-failure") != 0 && wcscmp(argv[3], L"--inject-arabic-failure") != 0 && wcscmp(argv[3], L"--inject-joined-failure") != 0 && wcscmp(argv[3], L"--inject-ligature-failure") != 0 && wcscmp(argv[3], L"--inject-paint-failure") != 0))
    {
        fputs("Usage: VT7.RendererProbe.exe --output <log path> [--inject-required-failure|--inject-font-failure|--inject-adapter-failure|--inject-fit-failure|--inject-arabic-failure|--inject-joined-failure|--inject-ligature-failure|--inject-paint-failure]\n", stderr);
        return 64;
    }
    Report report;
    report.text << "VT7 renderer capability and font probe 0.13\nNot Atlas rendering. GDI proof is unchanged.\n"
    ...
    if (SUCCEEDED(com))
    {
        ...
        try
        {
            ...
            VT7::FontProbe::Exercise(report.text, dwrite.Get(), std::wstring(argv[2]) + L".bmp",
                ...
                argc == 4 && wcscmp(argv[3], L"--inject-arabic-failure") == 0,
                argc == 4 && wcscmp(argv[3], L"--inject-joined-failure") == 0,
                argc == 4 && wcscmp(argv[3], L"--inject-ligature-failure") == 0,
                argc == 4 && wcscmp(argv[3], L"--inject-paint-failure") == 0);
