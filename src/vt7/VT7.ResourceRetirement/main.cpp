...
// Stable debugger entry point: x64 RCX=phase, EDX=iteration, R8B=live.
// Exporting and disabling inlining keeps it addressable without a pinned RVA.
extern "C" __declspec(dllexport) __declspec(noinline)
void VT7RetirementSample(const char* phase, uint32_t iteration, bool live) noexcept(false)
{
        ++sampleCount;
...
namespace
{
    void idleWait(DWORD targetMilliseconds, DWORD requestedMilliseconds)
    {
        const auto began = GetTickCount64();
        printf("IDLE_WAIT target_ms=%lu requested_ms=%lu begin_ms=%llu\n",
               targetMilliseconds, requestedMilliseconds, began);
        // The normal message loop continues. GetTickCount64 measures wall time,
        // so the collector must disable event breakpoints during these waits.
        pump(requestedMilliseconds);
        const auto ended = GetTickCount64();
        printf("IDLE_WAIT_END target_ms=%lu requested_ms=%lu begin_ms=%llu end_ms=%llu elapsed_ms=%llu\n",
               targetMilliseconds, requestedMilliseconds, began, ended, ended - began);
    }
    ...
    struct Api
    {
        ...
        void workload()
        {
            ...
            // starting client size before resetting the fixture.
...
int wmain(int argc, wchar_t** argv)
{
    ...
    try
    {
        if (argc < 3 || wcscmp(argv[2], L"reuse"))
            throw Failure{ "usage-native-dll-reuse", E_INVALIDARG };
        unsigned cycles = 25, failAt = 0;
        ...
        if (cycles != 25) throw Failure{ "retirement-requires-25-cycles", E_INVALIDARG };
        if (failAt > cycles) throw Failure{ "failure-cycle-outside-workload", E_INVALIDARG };
        printf("VT7 RESOURCE RETIREMENT 0.1; compiled %s %s; pid=%lu\n", __DATE__, __TIME__, GetCurrentProcessId());
        printf("CONFIG mode=reuse renderer=2 capture=1 power=0 probes=0 warmup=2 cycles=%u fail_at_cycle=%u\n", cycles, failAt);
        printf("RETIREMENT post_close_targets_ms=10000,90000,180000 wait_segments_ms=10000,80000,90000; wait_timing=wall_clock; debugger_pauses_count_toward_wait=1\n");
...
#undef LOAD
        ...
        windowClass.lpszClassName = L"VT7ResourceRetirement01";
        ...
        api.parent = CreateWindowExW(WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE | WS_EX_TRANSPARENT,
                                    windowClass.lpszClassName, L"VT7 resource retirement diagnostic", WS_POPUP,
        ...
        pump(250);
        VT7RetirementSample("pre-warmup", 0, false);
        for (unsigned iteration = 0; iteration < Warmup + cycles; ++iteration)
        {
            ...
            {
                pump(250);
                VT7RetirementSample(iteration + 1 < Warmup ? "warmup-live" : (iteration + 1 == Warmup ? "baseline-live" : "measured-live"), measured, true);
            }
            if (iteration + 1 == Warmup + cycles) api.closeSurface();
            pump(20);
        ...
        VT7RetirementSample("final-closed", cycles, false);
        idleWait(10000, 10000);
        VT7RetirementSample("final-closed-10s", cycles, false);
        idleWait(90000, 80000);
        VT7RetirementSample("final-closed-90s", cycles, false);
        idleWait(180000, 90000);
        VT7RetirementSample("final-closed-180s", cycles, false);
        constexpr unsigned expectedCreates = 1;
