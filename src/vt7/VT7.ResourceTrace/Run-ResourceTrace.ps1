...
$vt7Run = Join-Path (Join-Path $vt7Root 'Logs') ('resource-trace-' + [DateTime]::Now.ToString('yyyyMMdd-HHmmss') + '-' + [Guid]::NewGuid().ToString('N').Substring(0, 8))
$vt7Exit = 1
function Get-VT7TraceHash {
    param([string]$Path)
...
function Quote-VT7TraceArgument {
    param([string]$Value)
...
try {
    ...
    [IO.File]::WriteAllLines($vt7Summary, [string[]]@(
        'VT7 resource trace 0.3: existing lifetime 0.2 executable and native 0.3.5 DLL.',
        ...
        'Process-scoped CDB trace; reuse, 2 warm-up + 25 measured iterations, then final close + 10 seconds.',
    ...
    foreach ($vt7Line in [IO.File]::ReadAllLines($vt7Manifest)) {
        ...
        $vt7Seen[$vt7Relative] = $true
        if ((Get-VT7TraceHash (Join-Path $vt7Root $vt7Relative)) -ne $vt7Expected) { throw "Package checksum mismatch: $vt7Relative" }
    }
    # These identities also bind the private checkpoint RVA to the issued bytes.
    $vt7Pinned = @{
        ...
        'symbols/VT7.ResourceLifetime.pdb' = 'C2696000C6829C54DE5CA284379ED199E06E773A2ECE5308B29257A32B76F02B'
    ...
    foreach ($vt7Relative in $vt7Pinned.Keys) {
        if ((Get-VT7TraceHash (Join-Path $vt7Root $vt7Relative)) -ne $vt7Pinned[$vt7Relative]) { throw "Pinned input mismatch: $vt7Relative" }
    }
    foreach ($vt7Relative in @('preflight.cdb', 'startup.cdb', 'trace.cdb', 'Validate-ResourceTrace.ps1', 'Run-ResourceTrace.ps1')) {
    ...
    foreach ($vt7Relative in @('preflight.cdb', 'startup.cdb', 'trace.cdb')) {
        Copy-Item -LiteralPath (Join-Path $vt7Root $vt7Relative) -Destination $vt7Run
    }
    . (Join-Path $vt7Root 'Validate-ResourceTrace.ps1')
    if (-not $CdbPath) {
    ...
    foreach ($vt7DebuggerFile in @('cdb.exe', 'dbgeng.dll', 'dbghelp.dll', 'winext\ext.dll', 'winxp\exts.dll')) {
        ...
        if (Test-Path -LiteralPath $vt7DebuggerPath -PathType Leaf) {
            Add-Content -LiteralPath $vt7Summary -Value ('DEBUGGER file=' + $vt7DebuggerPath + ' version=' + [Diagnostics.FileVersionInfo]::GetVersionInfo($vt7DebuggerPath).FileVersion + ' SHA256=' + (Get-VT7TraceHash $vt7DebuggerPath))
        }
    }
...
using System.IO;
using System.Text;
public static class VT7TraceProcess
{
...
'@
    foreach ($vt7Stage in @('preflight', 'startup', 'trace')) {
        ...
        $vt7Arguments = '-G -y ' + (Quote-VT7TraceArgument (Join-Path $vt7Root 'symbols')) +
            ' -logo ' + (Quote-VT7TraceArgument $vt7DebuggerLog) +
            ' -cf ' + (Quote-VT7TraceArgument (Join-Path $vt7Run ($vt7Stage + '.cdb'))) +
            ' ' + (Quote-VT7TraceArgument (Join-Path $vt7Root 'VT7.ResourceLifetime.exe')) +
            ' ' + (Quote-VT7TraceArgument (Join-Path $vt7Root 'VT7.Native.dll')) + ' reuse --cycles 25'
        Add-Content -LiteralPath $vt7Summary -Value ('COMMAND ' + (Quote-VT7TraceArgument $CdbPath) + ' ' + $vt7Arguments)
        Write-Host "Running $vt7Stage..."
        $vt7Timeout = 600000
        $vt7Fallback = ''
        if ($vt7Stage -ne 'trace') { $vt7Timeout = 60000 }
        if ($vt7Stage -ne 'preflight') { $vt7Fallback = '.echo; .echo VT7_TRACE_ABORT_UNEXPECTED_STOP; .lastevent; .exr -1; r; k 0n24; lmv; q' }
        $vt7Code = [VT7TraceProcess]::Run($CdbPath, $vt7Arguments, $vt7Root, $vt7Combined, $vt7Stderr, $vt7Timeout, $vt7Fallback)
        ...
        if ($vt7Stage -eq 'preflight') {
            Assert-VT7TracePreflight $vt7Text
        ...
        elseif ($vt7Stage -eq 'startup') {
            Assert-VT7StartupControl $vt7Text
            ...
            Assert-VT7ResourceTrace $vt7Text
