VT7 OpenSSH-compatible known-host management KH01.4 package 0.8
================================================================

Package 0.5 is rejected on both Windows 7 test machines. Its disposable
removal check found that an unchanged .NET FileSecurity object was not applied
to the temporary replacement file, so the primary known_hosts ACL differed
after replacement. Version 0.12.1/package 0.6 corrected that failure and
passes the automated corpus on NESSY and TURTLE, but a live removal from the
owner's real known_hosts file stops safely at temporary-security. Version
0.12.2/package 0.7 compares owner, group, inheritance protection and exact
DACL entries before replacement rather than comparing the entire serialized
descriptor, whose auto-inheritance control bits can differ. The Windows 7
automated run then exposed an inconsistent inherited-ACL fixture that still
compared full SDDL after a structurally verified removal. Version 0.12.3/
package 0.8 corrects that assertion to match the production security contract.
A mismatch still stops before mutation and reports the mismatched component.
The changed key-row text has an explicit dark foreground on the white dialog.
