[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-KnownHosts-KH01-0.3-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-KnownHosts-KH01-0\.3-x64$') { throw "Invalid KH01.2 package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing KH01.2 package: $path" }
}
if (-not $NoBuild) { & (Join-Path $PSScriptRoot 'Build-VT7.ps1') -Configuration Release }

$hostPath = Join-Path $binaryRoot 'VT7.Host.exe'
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.10.1.0') { throw 'Expected VT7.Host 0.10.1.0.' }
...
$inputs = [ordered]@{
    ...
    'RUN-KNOWN-HOSTS-KH01-2.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-KNOWN-HOSTS-KH01-2.cmd'
    ...
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\KNOWN-HOSTS-KH01-2-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "KH01.2 package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-known-hosts-kh01-2-package-v1'
    package = $PackageName
    ...
    scope = 'KH01.2 read-only production trust using four default OpenSSH sources in direct and typed SSH.NET paths, with mandatory unknown-host fingerprint fallback and fail-closed changed, revoked, unreadable and policy states'
    networkConnections = $true
    credentialsRetained = $false
    typedSshEnabled = $true
    productionKnownHostsIntegration = $true
    knownHostsWriteEnabled = $false
...
foreach ($testPath in @($localTestRoot, $batchTestRoot)) {
    if (Test-Path -LiteralPath $testPath) { throw "Refusing to replace an existing KH01.2 test directory: $testPath" }
}
...
try {
    ...
    foreach ($runner in @('RUN-SSHNET-OVERLAY.cmd', 'RUN-KNOWN-HOSTS-KH01-2.cmd')) {
        $batchRunner = Join-Path $batchTestRoot $runner
        & (Join-Path $env:SystemRoot 'System32\cmd.exe') /d /c ('"' + $batchRunner + '"')
        if ($LASTEXITCODE -ne 0) { throw "Packaged KH01.2 launcher $runner failed with exit code $LASTEXITCODE." }
    }
}
finally {
...
if ($seen.Count -ne $expected.Count) { throw 'KH01.2 ZIP is missing one or more staged files.' }
