...
if (-not $ArchivePath) { $ArchivePath = Join-Path $repositoryRoot 'artifacts\vt7\packages\VT7-KnownHosts-KH01-0.1-x64.zip' }
$archiveFull = [IO.Path]::GetFullPath($ArchivePath)
if (-not (Test-Path -LiteralPath $archiveFull -PathType Leaf)) { throw "KH01.1 archive is missing: $archiveFull" }
...
foreach ($required in @('VT7.Host.exe','VT7.Native.dll','Test-VT7KnownHosts.ps1','RUN-KNOWN-HOSTS-KH01.cmd','README.txt','PACKAGE-MANIFEST.json','SHA256SUMS.txt','LICENSE.txt','NOTICE.md')) {
    if (-not $entries.ContainsKey($required)) { throw "KH01.1 archive is missing $required." }
}
$utf8 = New-Object Text.UTF8Encoding($false, $true)
$manifest = $utf8.GetString($entries['PACKAGE-MANIFEST.json']) | ConvertFrom-Json
if ($manifest.schema -ne 'vt7-known-hosts-kh01-package-v1' -or $manifest.applicationVersion -ne '0.9.2' -or
    $manifest.nativeAbi -ne 11 -or $manifest.networkConnections -ne $false -or
    $manifest.productionKnownHostsIntegration -ne $false) { throw 'KH01.1 manifest contract mismatch.' }
...
try {
    ...
    try {
        ...
        & (Join-Path $env:SystemRoot 'System32\cmd.exe') /d /c ('"' + (Join-Path $extractRoot 'RUN-KNOWN-HOSTS-KH01.cmd') + '"')
        if ($LASTEXITCODE -ne 0) { throw "Extracted KH01.1 launcher failed with exit code $LASTEXITCODE." }
    }
    finally {
