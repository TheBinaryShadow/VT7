[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-KnownHosts-KH01-0.5-x64',
    [switch]$NoBuild
)
...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.12.0.0') { throw 'Expected VT7.Host 0.12.0.0.' }
...
$inputs = [ordered]@{
    ...
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\KNOWN-HOSTS-KH01-4-README.txt'
...
$manifest = [ordered]@{
    ...
    scope = 'KH01.4 changed-key review, selected primary-user record removal with exact .old backup, and OpenSSH host-certificate CA/principal/validity/critical-option/revocation policy in direct and typed SSH.NET paths'
