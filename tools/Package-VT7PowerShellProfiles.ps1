[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-PowerShell-Profiles-0.6.6-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-PowerShell-Profiles-0\.6\.6-x64$') { throw "Invalid PowerShell-profile package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing PowerShell-profile package: $path" }
}
...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.6.6.0') { throw 'Expected VT7.Host 0.6.6.0.' }
...
$inputs = [ordered]@{
    ...
    'Test-VT7PowerShellProfiles.ps1' = Join-Path $PSScriptRoot 'Test-VT7PowerShellProfiles.ps1'
    'RUN-POWERSHELL-PROFILES.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-POWERSHELL-PROFILES.cmd'
    ...
    'RUN-VT7-POWERSHELL-7.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-VT7-POWERSHELL-7.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\POWERSHELL-PROFILES-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "PowerShell-profile package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-powershell-profiles-package-v1'
    package = $PackageName
    ...
    scope = 'Milestone 3B.2 Windows 7 PowerShell qualification plus HwndHost keyboard-sink ownership for terminal navigation keys through pinned WinPTY 0.4.3'
    winPtyVersion = '0.4.3'
}
...
try {
    ...
    & (Join-Path $PSScriptRoot 'Test-VT7PowerShellProfiles.ps1') -Configuration Release -BinaryDirectory $packageRoot -OutputDirectory $localTestRoot -AllowMissingPowerShell7
}
finally {
...
if ($seen.Count -ne $expected.Count) { throw 'PowerShell-profile ZIP is missing one or more staged files.' }
