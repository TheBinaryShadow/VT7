[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-Session-Ownership-0.4.0-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-Session-Ownership-0\.4\.0-x64$') { throw "Invalid session-ownership package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing session-ownership package: $path" }
}
...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.4.0.0') { throw 'Expected VT7.Host 0.4.0.0.' }
...
$inputs = [ordered]@{
    ...
    'RUN-SESSION-OWNERSHIP.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-SESSION-OWNERSHIP.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\SESSION-OWNERSHIP-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Session-ownership package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-session-ownership-package-v1'
    package = $PackageName
    ...
    scope = 'ABI 11 terminal document/view lifetime and managed fake-transport session ownership'
}
...
if ($seen.Count -ne $expected.Count) { throw 'Session-ownership ZIP is missing one or more staged files.' }
