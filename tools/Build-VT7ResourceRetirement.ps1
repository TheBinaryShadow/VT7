...
$outputDirectory = Join-Path $repositoryRoot "artifacts\vt7\diagnostics\resource-retirement-build-$buildId"
...
$executablePath = Join-Path $outputDirectory 'VT7.ResourceRetirement.exe'
$symbolPath = Join-Path $outputDirectory 'VT7.ResourceRetirement.pdb'
$arguments = @(
    ...
    "/Fe$executablePath", "/Fo$(Join-Path $outputDirectory 'resource-retirement.obj')",
...
$buildManifest = [ordered]@{
    Diagnostic = 'VT7.ResourceRetirement'
...
Write-Host "Building standalone VT7 resource retirement diagnostic 0.1: $outputDirectory"
