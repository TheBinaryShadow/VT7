[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-Session-Outbound-0.3.7-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-Session-Outbound-0\.3\.7-x64$') { throw "Invalid session-outbound package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing session-outbound package: $path" }
}
...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.3.7.0') { throw 'Expected VT7.Host 0.3.7.0.' }
...
$inputs = [ordered]@{
    ...
    'RUN-SESSION-OUTBOUND.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-SESSION-OUTBOUND.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\SESSION-OUTBOUND-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Session-outbound package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-session-outbound-package-v1'
    package = $PackageName
    ...
    scope = 'Generation-checked outbound queue and native child-HWND input/resize adapter'
}
...
if ($seen.Count -ne $expected.Count) { throw 'Session-outbound ZIP is missing one or more staged files.' }
