...
if (-not $ArchivePath) { $ArchivePath = Join-Path $repositoryRoot 'artifacts\vt7\packages\VT7-Legacy-Win7-0.1-x64.zip' }
...
foreach ($required in @('VT7.Host.exe','VT7.Native.dll','shim/ssh.exe','shim/ssh-system.exe',
    'RUN-VT7-LEGACY-BASELINE.cmd','RUN-VT7-COMMAND-PROMPT.cmd','KH01-4-README.txt',
...
if ($manifest.schema -ne 'vt7-legacy-win7-package-v1' -or
    $manifest.package -ne 'VT7-Legacy-Win7-0.1-x64' -or
    $manifest.applicationVersion -ne '0.12.4' -or
    $manifest.nativeAbi -ne 11 -or
    $manifest.baselineArchiveSha256 -ne '16509A782C8EE629E74F8CE4D4FE11E265C93763BB2E75036AC3C5FEE6920AEE' -or
...
try {
    ...
    if ([Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $extractRoot 'VT7.Host.exe')).FileVersion -ne '0.12.4.0') {
        throw 'Extracted legacy candidate host version is not 0.12.4.0.'
    }
