[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-KnownHosts-KH01-0.1-x64',
    [switch]$NoBuild
)
...
$noticeRoot = Join-Path $dependencyRoot 'sshnet-source-notices-7b2fd3dbf2c86a80a7b06cea020aa5f821c9902e'
...
if ($PackageName -notmatch '^VT7-KnownHosts-KH01-0\.1-x64$') { throw "Invalid KH01.1 package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing KH01.1 package: $path" }
}
if (-not $NoBuild) { & (Join-Path $PSScriptRoot 'Build-VT7.ps1') -Configuration Release }

$hostPath = Join-Path $binaryRoot 'VT7.Host.exe'
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.9.2.0') { throw 'Expected VT7.Host 0.9.2.0.' }
...
$inputs = [ordered]@{
    ...
    'RUN-KNOWN-HOSTS-KH01.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-KNOWN-HOSTS-KH01.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\KNOWN-HOSTS-KH01-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "KH01.1 package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-known-hosts-kh01-package-v1'
    package = $PackageName
    ...
    scope = 'Disconnected KH01.1 OpenSSH known-host parser, matcher, raw-key trust resolver, bounded arbitrary-input corpus and ssh-keygen differential oracle'
    openSshOracle = 'Microsoft Win32-OpenSSH 10.0p2 required on the Windows 7 target'
    networkConnections = $false
    credentialsRetained = $false
    typedSshEnabled = $false
    productionKnownHostsIntegration = $false
...
foreach ($testPath in @($localTestRoot, $batchTestRoot)) {
    if (Test-Path -LiteralPath $testPath) { throw "Refusing to replace an existing KH01.1 test directory: $testPath" }
}
...
try {
    ...
    $batchRunner = Join-Path $batchTestRoot 'RUN-KNOWN-HOSTS-KH01.cmd'
    & (Join-Path $env:SystemRoot 'System32\cmd.exe') /d /c ('"' + $batchRunner + '"')
    if ($LASTEXITCODE -ne 0) { throw "Packaged KH01.1 batch launcher failed with exit code $LASTEXITCODE." }
}
finally {
...
if ($seen.Count -ne $expected.Count) { throw 'KH01.1 ZIP is missing one or more staged files.' }
