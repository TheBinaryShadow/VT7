[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-H01-0.4-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-H01-0\.4-x64$') { throw "Invalid H01 package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing H01 package: $path" }
}
if (-not $NoBuild) { & (Join-Path $PSScriptRoot 'Build-VT7.ps1') -Configuration Release }

$hostPath = Join-Path $binaryRoot 'VT7.Host.exe'
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.7.3.0') { throw 'Expected VT7.Host 0.7.3.0.' }
...
$inputs = [ordered]@{
    ...
    'RUN-H01.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-H01.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\H01-README.txt'
...
foreach ($source in $inputs.Values) { if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "H01 package input is missing: $source" } }
foreach ($shim in @('ssh.exe', 'ssh-system.exe')) {
    $path = Join-Path $binaryRoot ('shim\' + $shim)
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "H01 shim is missing: $path" }
}
...
$manifest = [ordered]@{
    ...
    scope = 'H01 Windows 7 typed-command native shim, authenticated local named-pipe handshake, exact system fallback and committed WinPTY ordering barrier diagnostic'
    networkConnections = $false
    embeddedSshEnabled = $false
    winPtyVersion = '0.4.3'
}
...
$resolvedPackageParent = [IO.Path]::GetFullPath($packageParent).TrimEnd([IO.Path]::DirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar
$resolvedLocalTestRoot = [IO.Path]::GetFullPath($localTestRoot)
if (-not $resolvedLocalTestRoot.StartsWith($resolvedPackageParent, [StringComparison]::OrdinalIgnoreCase)) {
    throw "Refusing cleanup outside the H01 package directory: $resolvedLocalTestRoot"
}
if (Test-Path -LiteralPath $localTestRoot) { throw "Refusing to replace an existing H01 test directory: $localTestRoot" }
try { & (Join-Path $PSScriptRoot 'Test-VT7H01.ps1') -Configuration Release -BinaryDirectory $packageRoot -OutputDirectory $localTestRoot -AllowMissingPowerShell7 }
finally { if (Test-Path -LiteralPath $localTestRoot) { Remove-Item -LiteralPath $localTestRoot -Recurse -Force } }
...
if ($seen.Count -ne $expected.Count) { throw 'H01 ZIP is missing one or more staged files.' }
