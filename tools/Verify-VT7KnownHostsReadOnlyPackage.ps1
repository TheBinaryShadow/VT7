...
if (-not $ArchivePath) { $ArchivePath = Join-Path $repositoryRoot 'artifacts\vt7\packages\VT7-KnownHosts-KH01-0.3-x64.zip' }
$archiveFull = [IO.Path]::GetFullPath($ArchivePath)
if (-not (Test-Path -LiteralPath $archiveFull -PathType Leaf)) { throw "KH01.2 archive is missing: $archiveFull" }
...
foreach ($required in @('VT7.Host.exe','VT7.Native.dll','shim/ssh.exe','shim/ssh-system.exe',
    'Test-VT7KnownHosts.ps1','Test-VT7SshOverlay.ps1','RUN-KNOWN-HOSTS-KH01-2.cmd',
    'RUN-VT7-COMMAND-PROMPT.cmd','README.txt','PACKAGE-MANIFEST.json','SHA256SUMS.txt','LICENSE.txt','NOTICE.md')) {
    if (-not $entries.ContainsKey($required)) { throw "KH01.2 archive is missing $required." }
}
$utf8 = New-Object Text.UTF8Encoding($false, $true)
$manifest = $utf8.GetString($entries['PACKAGE-MANIFEST.json']) | ConvertFrom-Json
if ($manifest.schema -ne 'vt7-known-hosts-kh01-2-package-v1' -or $manifest.applicationVersion -ne '0.10.1' -or
    $manifest.nativeAbi -ne 11 -or $manifest.networkConnections -ne $true -or
    $manifest.productionKnownHostsIntegration -ne $true -or $manifest.knownHostsWriteEnabled -ne $false -or
    $manifest.typedSshEnabled -ne $true) { throw 'KH01.2 manifest contract mismatch.' }
...
try {
    ...
    if ([Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $extractRoot 'VT7.Host.exe')).FileVersion -ne '0.10.1.0') {
        throw 'Extracted KH01.2 host version is not 0.10.1.0.'
    }
    ...
    try {
        ...
        & (Join-Path $env:SystemRoot 'System32\cmd.exe') /d /c ('"' + (Join-Path $extractRoot 'RUN-KNOWN-HOSTS-KH01-2.cmd') + '"')
        if ($LASTEXITCODE -ne 0) { throw "Extracted KH01.2 launcher failed with exit code $LASTEXITCODE." }
    }
    finally {
