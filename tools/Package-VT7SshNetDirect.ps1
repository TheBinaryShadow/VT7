...
$inputs = [ordered]@{
    ...
    'RUN-SSHNET-FOUNDATION.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-SSHNET-FOUNDATION.cmd'
    'RUN-VT7-SSHNET-DIRECT.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-VT7-SSHNET-DIRECT.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\SSHNET-DIRECT-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "SSH.NET direct package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-sshnet-direct-package-v1'
    package = $PackageName
    ...
    scope = 'Milestone 5 direct SSH.NET root transport with mandatory out-of-band SHA256 trust, private-key/password authentication, PTY stream, input, authoritative resize and stream-first shutdown'
...
if ($seen.Count -ne $expected.Count) { throw 'SSH.NET direct ZIP is missing one or more staged files.' }
