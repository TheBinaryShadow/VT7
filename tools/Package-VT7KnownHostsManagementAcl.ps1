[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-KnownHosts-KH01-0.6-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-KnownHosts-KH01-0\.6-x64$') { throw "Invalid KH01.4 package name: $PackageName" }
...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.12.1.0') { throw 'Expected VT7.Host 0.12.1.0.' }
...
$inputs = [ordered]@{
    ...
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\KNOWN-HOSTS-KH01-4-ACL-README.txt'
...
$manifest = [ordered]@{
    ...
    scope = 'KH01.4 changed-key review and host-certificate policy with Windows 7 ACL-corrected selected user-record removal, pre-replacement owner/group/DACL verification and exact .old backup'
