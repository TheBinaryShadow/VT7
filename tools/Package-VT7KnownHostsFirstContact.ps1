[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-KnownHosts-KH01-0.4-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-KnownHosts-KH01-0\.4-x64$') { throw "Invalid KH01.3 package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing KH01.3 package: $path" }
}
if (-not $NoBuild) { & (Join-Path $PSScriptRoot 'Build-VT7.ps1') -Configuration Release }

$hostPath = Join-Path $binaryRoot 'VT7.Host.exe'
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.11.0.0') { throw 'Expected VT7.Host 0.11.0.0.' }
...
$inputs = [ordered]@{
    ...
    'RUN-KNOWN-HOSTS-KH01-3.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-KNOWN-HOSTS-KH01-3.cmd'
    ...
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\KNOWN-HOSTS-KH01-3-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "KH01.3 package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-known-hosts-kh01-3-package-v1'
    package = $PackageName
    ...
    scope = 'KH01.3 generation-safe unknown-host discovery with Cancel, Connect once and verified durable Trust and connect for direct and typed SSH.NET paths; changed, revoked, unreadable and policy states remain fail-closed'
...
foreach ($testPath in @($localTestRoot, $batchTestRoot)) {
    if (Test-Path -LiteralPath $testPath) { throw "Refusing to replace an existing KH01.3 test directory: $testPath" }
}
...
try {
    ...
    foreach ($runner in @('RUN-SSHNET-OVERLAY.cmd', 'RUN-KNOWN-HOSTS-KH01-3.cmd')) {
        $batchRunner = Join-Path $batchTestRoot $runner
        & (Join-Path $env:SystemRoot 'System32\cmd.exe') /d /c ('"' + $batchRunner + '"')
        if ($LASTEXITCODE -ne 0) { throw "Packaged KH01.3 launcher $runner failed with exit code $LASTEXITCODE." }
    }
}
finally {
...
if ($seen.Count -ne $expected.Count) { throw 'KH01.3 ZIP is missing one or more staged files.' }
