[CmdletBinding()]
param([string]$PackageName = 'VT7-Legacy-Win7-0.1-x64')
$ErrorActionPreference = 'Stop'
Set-StrictMode -Version 3.0

$repositoryRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$packageParent = Join-Path $repositoryRoot 'artifacts\vt7\packages'
$baseline = Join-Path $packageParent 'VT7-KnownHosts-KH01-0.8-x64.zip'
$baselineSha256 = '16509A782C8EE629E74F8CE4D4FE11E265C93763BB2E75036AC3C5FEE6920AEE'
...
if ($PackageName -ne 'VT7-Legacy-Win7-0.1-x64') { throw 'Unexpected legacy candidate name.' }
...
if ([Diagnostics.FileVersionInfo]::GetVersionInfo($hostPath).FileVersion -ne '0.12.4.0') {
    throw 'Build the 0.12.4 Release host before packaging.'
}

Add-Type -AssemblyName System.IO.Compression.FileSystem
New-Item -ItemType Directory -Path $packageRoot -Force | Out-Null
[IO.Compression.ZipFile]::ExtractToDirectory($baseline, $packageRoot)
Move-Item -LiteralPath (Join-Path $packageRoot 'README.txt') -Destination (Join-Path $packageRoot 'KH01-4-README.txt')
...
Copy-Item -LiteralPath (Join-Path $repositoryRoot 'src\vt7\packaging\LEGACY-WIN7-0-1-README.txt') -Destination (Join-Path $packageRoot 'README.txt')
...
$manifest.schema = 'vt7-legacy-win7-package-v1'
$manifest.package = $PackageName
$manifest.createdUtc = [DateTime]::UtcNow.ToString('o')
$manifest.applicationVersion = '0.12.4'
$manifest.scope = 'Pre-WMF 5.1 Windows 7 SP1 baseline with actual Windows PowerShell version discovery and a cmd.exe-driven diagnostic launcher'
$manifest | Add-Member -NotePropertyName baselineArchiveSha256 -NotePropertyValue $baselineSha256
$manifest | Add-Member -NotePropertyName windowsPowerShell2Qualified -NotePropertyValue $false
$manifest | Add-Member -NotePropertyName legacyCmdRunner -NotePropertyValue 'RUN-VT7-LEGACY-BASELINE.cmd'
