...
if (-not $ArchivePath) { $ArchivePath = Join-Path $repositoryRoot 'artifacts\vt7\packages\VT7-KnownHosts-KH01-0.8-x64.zip' }
...
if ($manifest.schema -ne 'vt7-known-hosts-kh01-4-package-v1' -or $manifest.applicationVersion -ne '0.12.3' -or
...
try {
    ...
    if ([Diagnostics.FileVersionInfo]::GetVersionInfo((Join-Path $extractRoot 'VT7.Host.exe')).FileVersion -ne '0.12.3.0') {
        throw 'Extracted KH01.4 host version is not 0.12.3.0.'
    }
