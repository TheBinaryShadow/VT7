[CmdletBinding()]
param(
    [string]$PackageName = 'VT7-SSHNET-Overlay-0.3-x64',
    [switch]$NoBuild
)
...
if ($PackageName -notmatch '^VT7-SSHNET-Overlay-0\.3-x64$') { throw "Invalid SSH.NET overlay package name: $PackageName" }
foreach ($path in @($packageRoot, $archivePath)) {
    if (Test-Path -LiteralPath $path) { throw "Refusing to replace an existing SSH.NET overlay package: $path" }
}
...
$inputs = [ordered]@{
    ...
    'RUN-VT7-SSHNET-OVERLAY.cmd' = Join-Path $repositoryRoot 'src\vt7\packaging\RUN-VT7-SSHNET-OVERLAY.cmd'
    'README.txt' = Join-Path $repositoryRoot 'src\vt7\packaging\SSHNET-OVERLAY-README.txt'
...
foreach ($source in $inputs.Values) {
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "SSH.NET overlay package input is missing: $source" }
}
...
$manifest = [ordered]@{
    schema = 'vt7-sshnet-overlay-package-v1'
    package = $PackageName
    ...
    scope = 'Milestone 5 typed SSH overlay coordinator with authenticated shim, committed WinPTY barrier, structured trust/authentication prompt, SSH.NET PTY overlay and deterministic return to the originating local shell'
...
foreach ($testPath in @($localTestRoot, $batchTestRoot)) {
    if (Test-Path -LiteralPath $testPath) { throw "Refusing to replace an existing SSH.NET overlay test directory: $testPath" }
}
...
try {
    ...
    $batchRunner = Join-Path $batchTestRoot 'RUN-SSHNET-OVERLAY.cmd'
    & (Join-Path $env:SystemRoot 'System32\cmd.exe') /d /c ('"' + $batchRunner + '"')
    if ($LASTEXITCODE -ne 0) { throw "Packaged typed SSH overlay batch launcher failed with exit code $LASTEXITCODE." }
}
finally {
...
if ($seen.Count -ne $expected.Count) { throw 'SSH.NET overlay ZIP is missing one or more staged files.' }
