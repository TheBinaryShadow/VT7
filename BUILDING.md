...
PowerShell 5.1 is a development and existing KH01.4 automated-runner
requirement, not an installed-runtime requirement for VT7. A Windows 7 SP1
machine with .NET Framework 4.8 and Windows PowerShell 2.0 is the legacy test tier; see
...
six LEOPARD baseline stages, including the no-external-shim check. The owner
confirms live typed and direct SSH connections and return to the local prompt;
typing, resize, disconnect, `htop` and `nano` also work in live SSH. The
pre-WMF Command Prompt and SSH runtime scope is accepted. The older KH01.4
PowerShell test runner cannot validate the PowerShell 2.0 tier; current source
...
- .NET Framework 4.8. VT7 0.12.5 uses SSH.NET
